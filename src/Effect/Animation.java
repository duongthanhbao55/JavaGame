...
public class Animation {
	...
	public Animation(Animation animation) {
		// biến nguyên thủy ( khí gán bằng chỉ lấy giá trị, không sử dụng vùng nhớ của
		// biến gán bằng cho nó)
		...
		// các câu lệnh dưới sử dụng toán tử new để tránh trùng địa chỉ của 2 đối tượng
		// khi gán bằng cho nhau(kiểu tham chiếu)
	...
	public void Update(long currentTime) {
		...
		if (this.beginTime == 0)
			...
			if (currentTime - beginTime > delayFrames.get(currentFrame))// kiểm tra 'thời gian' từ lúc 'bắt đầu' 1 khung
																		// hình cho đến khi
																		// 'kết thúc' 1 khung hình có bằng
																		// delayTime(thời gian chuyển giao giữa 2 frame)
																		// Nếu bằng nhau thì có thể chuyển sang frame
																		// tiếp theo và set lại begin time bằng thời
																		// gian hiện tại
			{
				nextFrame();
				beginTime = currentTime;
	...
	private void nextFrame() {
		...
		if (ignoreFrames.get(currentFrame))
			nextFrame();// Nếu khung hình hiện tại được set bỏ qua chạy lại hàm
	}
	...
	public void draw(int x, int y, int width, int height, Graphics g) {
		...
		g.drawImage(image, (int) (x - image.getWidth() / 2 * Game.SCALE),
				(int) (y - image.getHeight() / 2 * Game.SCALE), width, height, null);
	}

	public void drawNormal(int x, int y, int width, int height, Graphics g) {
		BufferedImage image = getCurrentImage();
