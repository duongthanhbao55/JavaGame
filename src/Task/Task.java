...
//	}
	public static void TaskGet(Player player, final short npcsTemplateId) {
		...
		case 0:
			Confirm.OpenComfirmUI(player, npcsTemplateId, Talk.getTask(0,0),new String[] {Text.get(0, 0),Text.get(0, 1)});		
		}
	}
	...
	public static void upNextTask(final Player player, Playing playing) {
		...
		if (isFinishTask(player)) {
			System.out.println("DONE TASK, THANKS FOR PLAYING");
		...
		for (EnemyTemplate e : EnemyManager.arrEnemyTemplate) {
			if (Task.isExtermination(player, e)) {
				break;
			}
		}
	}
	...
	public static void doTask(Player player, Playing playing) {
		...
		case 1:
			if (NightBorne.getExtermination()) {
				...
				if (player.getCtaskCount() == Game.taskTemplates[player.getCtaskId()].counts[player.getCtaskIndex()
						+ 1]) {
